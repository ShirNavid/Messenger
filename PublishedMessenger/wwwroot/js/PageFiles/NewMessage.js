...
globalThis.pageFunctions["NewMessage"] = function () {
    ...
    (function setToolsNavbar() {
        toolsNavbar.setTools("NewMessage");
    })();

    (function setToolsNavbarMenuListItemSelected() {
        toolsNavbarMenuList.setItemSelected("New message");
    })();
    ...
    (function setSendFunction() {
        ...
        globalThis.sendFunction = function () {
            ...
            iframe.contentDocument.sendNewMessage(title, text, date, time, fileName, fileContent, fileType, mainAccountId, secondaryAccountIds);
        }

        toolsNavbar.setSendFunctionName("sendFunction");
    ...
    (function setSelectAccountProps() {
        ...
        extendedAddEventListener(iframe, "load", function () {
            ...
            selectAccountManager.setProps(0, senderId, 0);
    ...
    (function setSelectFunction() {
        globalThis.selectionFunction = function () {
            ...
            const mainAccountData = receiversList.getMainContact();
            const secondaryAccountsData = receiversList.getSecondaryContacts();
            if (accountData == null) {
                toolsNavbar.setSetAsMainContactEnabality(false);
                toolsNavbar.setAddAsSecondaryContactEnabality(false);
            }
            else {
                try {
                    if (accountData.id == mainAccountData.id || secondaryAccountsData.some(c => c.id == accountData.id)) {
                        toolsNavbar.setSetAsMainContactEnabality(false);
                        toolsNavbar.setAddAsSecondaryContactEnabality(false);
                    }
                    else {
                        toolsNavbar.setSetAsMainContactEnabality(true);
        ...
        toolsNavbar.setSelectAccountSelectionFunction("selectionFunction");

    })();

    (function setMainContactFunction() {

        globalThis.mainContactFunction = function () {
            const toolsNavbar = document.querySelector("tools-navbar");
            const iframe = document.querySelector("iframe");
            const accountData = toolsNavbar.getSelectedAccountData();
            const receiversList = iframe.contentDocument.querySelector("receivers-list");
        ...
        toolsNavbar.setSetAsMainContactFunction("mainContactFunction");
    ...
    (function setSecondaryContactFunction() {
        ...
        toolsNavbar.setAddAsSecondaryContactFunction("secondaryContactFunction");
